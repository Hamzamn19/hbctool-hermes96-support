...
def dump(hbc, path, force=False):
    ...
    def json_serializer(obj):
        if isinstance(obj, bytes):
            return obj.hex() # حول البايتات إلى نص سداسي عشري (hex)
        raise TypeError(f'Object of type {obj.__class__.__name__} is not JSON serializable')

    # استخدم الدالة المساعدة وأضف مسافة بادئة لتجميل الناتج
    json.dump(hbc.getObj(), f, default=json_serializer, indent=4)
    f.close()
    ...
    ss = []
    print(f"[DEBUG] Dumping {functionCount} functions...")
